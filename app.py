...
# Absolute path to store files is required on PythonAnywhere environment
...
def upload():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            params = {'resize_mode':resize_mode, 'width': width, 'height': height, 'fx': fx, 'fy': fy}
...
def resize_image():
    ...
    if params['resize_mode'] == 'ratio_mode':
        fx = float(params['fx'])
        fy = float(params['fy'])
        res = resizer_ratio(img, fx = fx, fy = fy)
    else: # defined_mode
